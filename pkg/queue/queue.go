...
type (
	...
	PublishingQueue Queue[PublishingJob]

	AdvertisementPublishingQueue Queue[schema.Advertisement]
...
func NewPublishingQueuePoller(queue PublishingQueue, publisher publisher.AsyncPublisher, opts ...Option) (*PublishingQueuePoller, error) {
	...
	return NewQueuePoller(
		queue,
		BatchJobHandler(func(ctx context.Context, jobs []WithID[PublishingJob]) map[string]error {
			errs := make(map[string]error, len(jobs))
			for _, job := range jobs {
...
func NewAdvertisementPublishingQueuePoller(queue AdvertisementPublishingQueue, advertisementPublisher *publisher.AdvertisementPublisher, opts ...Option) (*AdvertisementPublishingQueuePoller, error) {
	return NewQueuePoller(
		queue,
		BatchJobHandler(NewAdvertisementPublishingJobHandler(advertisementPublisher).Handle),
		opts...)
}
