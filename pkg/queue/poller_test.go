...
type mockQueue struct {
	mu              sync.Mutex
	jobs            []queue.WithID[testJob]
	jobsWaiting     chan struct{}
	readCount       int
	deletedJobs     map[string]struct{}
	releasedJobs    map[string]struct{}
	readBehavior    func(ctx context.Context, maxJobs int) ([]queue.WithID[testJob], error)
...
func (m *mockQueue) Queue(ctx context.Context, job testJob) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.jobs = append(m.jobs, queue.WithID[testJob]{ID: job.id, Job: job})
	select {
...
func (m *mockQueue) Read(ctx context.Context, maxJobs int) ([]queue.WithID[testJob], error) {
...
func TestQueuePollerJobHandlerSuccess(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		handler := queue.JobHandler(func(ctx context.Context, job testJob) error {
			processedJobs = append(processedJobs, job)
			return nil
		})

		poller, err := queue.NewQueuePoller(
			q,
			handler,
			queue.WithJobBatchSize(10),
		...
		for _, job := range testJobs {
			if err := q.Queue(context.Background(), job); err != nil {
		...
		releasedJobs := q.getReleasedJobs()
...
func TestQueuePollerBatchJobHandlerMixedResults(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		handler := queue.BatchJobHandler(func(ctx context.Context, jobs []queue.WithID[testJob]) map[string]error {
...
func TestQueuePollerMaxBatchSize(t *testing.T) {
	...
	_, err := queue.NewQueuePoller(
		q,
		handler,
		queue.WithJobBatchSize(20), // max is 10
		queue.WithConcurrency(1),
	)
