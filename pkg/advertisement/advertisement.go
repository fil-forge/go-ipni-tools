...
func ShardCID(provider peer.AddrInfo, caveats assert.LocationArguments) (*cid.Cid, error) {
	...
	for _, addr := range provider.Addrs {
		...
		for _, location := range caveats.Location {
			...
			if !urlRegex.MatchString(location.URL().String()) {
				continue
			}
			// get matching components of the location
			matches := urlRegex.FindStringSubmatch(location.URL().String())
			...
			if urlRegex.SubexpIndex("blob") != -1 {
				...
				if urlRegex.SubexpIndex("blobCID") == -1 {
					...
					if caveats.Content.String() == blob.String() {
						return nil, nil
					}
			...
			if cid.NewCidV1(cid.Raw, caveats.Content).Equals(blobCID) {
				return nil, nil
			}
